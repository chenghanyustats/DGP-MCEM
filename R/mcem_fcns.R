...
sample_sig2_subj <- function(multi_y, x_vec, theta, Sigma,
                             ga_shape = 5, ga_rate = 5) {
    n_subj <- ncol(multi_y)
    n <- nrow(multi_y)
    
    quadinv_lst <- lapply(1:ncol(multi_y), function(i) {
        quad.form.inv(Sigma[[s]] + diag(n), multi_y[, s])
    })
    quadinvsum <- sum(unlist(quadinv_lst))
    
    invgamma::rinvgamma(1, shape = ga_shape + (n * n_subj) / 2,
                        rate = ga_rate + (1 / 2) * quadinvsum)
}
...
log_dgbeta_prior <- function(shape1, shape2, t, 
    ...
    if (n_mixture == 1) {
        if (shape1 != 1 || shape2 != 1) {
            return(log_dgbeta(t, shape1 = shape1, shape2 = shape2,
                              a = a, b = b))
        }
    } else {
        # log_prior_den <- 0
        # for (m in 1:n_mixture) {
        #     log_prior_den <- log_prior_den + log_dgbeta(t[m], 
        #                                                 shape1 = shape1[m],
        #                                                 shape2 = shape2[m],
        #                                                 a = a[m], b = b[m])
        # }
        ...
        for (m in 1:n_mixture) {
            prior_den <- prior_den +
                mixture_prob[m] * exp(log_dgbeta(t, shape1 = shape1[m],
                                                 shape2 = shape2[m],
                                                 a = a, b = b))
        }
...
log_post_den_given_sig_matern <- function(t, x, theta, y, Kff, sig, nu,
                                          shape1, shape2, a, b, mixture_prob = NULL) {
    
    
    Kdf <- computeCovDer1(idx1 = t, idx2 = x, tau = theta[1], l = theta[2],
                          ker_der1_fcn = matern_ker_der1, nu = nu) 
    Kdd <- computeCovDer2(idx1 = t, tau = theta[1], l = theta[2], 
                          ker_der2_fcn = matern_ker_der2, nu = nu)
...
log_mar_lik_gp_der_mc_sig2 <- function(theta, y, x_vec, der_mc_mat, 
    ...
    den_value_lst_new <- lapply(1:ncol(der_mc_mat), function(m) {
        ...
        if (is.multi) {
            ...
            Psi <- sample_sig[m] ^ 2 * (Sigma + diag(n+0.001))
            Psi <- (Psi + t(Psi)) / 2
        } else {
            Sigma <- Kff - crossprod(Kdf_lst_new[[m]]) / ((theta[1] / theta[2]) ^ 2)
            Psi <- sample_sig[m] ^ 2 * (Sigma + diag(n+0.001))
...
log_mar_lik_gp_der_mc_sig2_matern <- function(theta, y, x_vec, der_mc_mat, 
                                              H0, sample_sig,
                                              a_h = 1, b_h = 1, nu,
                                              is.h.par = TRUE,
                                              is.multi = FALSE) {
    ...
    # Kff <- se_ker(H0 = H0, tau = theta[1], h = theta[2])
    Kff <- matern_ker(H0 = H0, nu = nu, tau = theta[1], l = theta[2])
    
    Kdf_lst <- as.list(data.frame(apply(der_mc_mat, 2, computeCovDer1, 
                                        idx2 = x_vec, 
                                        ker_der1_fcn = matern_ker_der1,
                                        tau = theta[1], l = theta[2],
                                        nu = nu)))
    ...
    den_value_lst_new <- lapply(1:ncol(der_mc_mat), function(m) {
        ...
        if (is.multi) {
            ...
            if (nu == 1.5) {
                Sigma <- Kff - crossprod(Kdf_lst_new[[m]]) / (3 * theta[1] ^ 2 / (theta[2] ^ 2))
            } 
            if (nu == 2.5) {
                Sigma <- Kff - crossprod(Kdf_lst_new[[m]]) / (5 * theta[1] ^ 2 / (3 * theta[2] ^ 2))
            } 
            if (nu == 0.5) {
                Sigma <- Kff + crossprod(Kdf_lst_new[[m]]) / (theta[1] ^ 2 / theta[2] ^ 2)
            }
            # Sigma <- Kff - crossprod(Kdf_lst_new[[m]]) / ((theta[1] / theta[2]) ^ 2)
            Psi <- sample_sig[m] ^ 2 * (Sigma + diag(n+0.001))
            Psi <- (Psi + t(Psi)) / 2
...
get_pp_t_sig <- function(yJ, x, x_test, idx_der, sample_sig, tau, h) {
    ...
    mean_var_lst <- lapply(1:mm, function(i) {
        ...
        Kdfnew <- computeCovDer1(idx1 = idx_der[i, ], idx2 = x_test, 
                                 tau = tau_new, h = h)
        return(cond_norm_mean_var(mean_vec_1 = 0, mean_vec_2 = 0, obs_2 = yJ,
                                  cov_mat_1 = Kfnewfnew + diag(sample_sig[i]^2, len_test),
                                  cov_mat_2 = K_joint,
                                  cov_mat_12 = t(rbind(Kffnew, Kdfnew))))
    })
    
    pred_y_der_mat <- sapply(mean_var_lst, function(x) {
        mvnfast::rmvn(1, mu = x$condMean,
                      sigma = x$condVar)})
    
    PI_y_der <- apply(pred_y_der_mat, 1, quantile, prob = c(0.025, 0.975))
    mu_test_der_mean <- apply(pred_y_der_mat, 1, mean)
    
    return(list(pred_y = pred_y_der_mat, ci_low = PI_y_der[1, ],
                ci_high = PI_y_der[2, ],
                mu_test = mu_test_der_mean))
}
...
get_pi_t_sig_matern <- function(yJ, x, x_test, idx_der, sample_sig, tau, l, nu = 1.5) {
